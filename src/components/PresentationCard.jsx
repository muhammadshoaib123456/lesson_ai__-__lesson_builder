...
/**
 * Instagram-like bookmark icon:
 * - default: white bg + black border
 * - saved: yellow fill
 */
function BookmarkIcon({ saved }) {
  const fill = saved ? "#FFD54A" : "#FFFFFF";
  const stroke = saved ? "#FFD54A" : "#000000";
  return (
    <svg width="22" height="22" viewBox="0 0 24 24" aria-hidden="true">
      <circle cx="12" cy="12" r="11" fill={fill} stroke={stroke} />
      <path
        d="M8 6h8a1 1 0 0 1 1 1v11l-5-3-5 3V7a1 1 0 0 1 1-1z"
        fill={saved ? "#8A6A00" : "none"}
        stroke={saved ? "#8A6A00" : "#000000"}
        strokeWidth="1.5"
...
export default function PresentationCard({
  p,
  cardHeight,
  initiallySaved = false,        // ⬅️ NEW: allow parent to set initial saved state
  onSavedChange,                 // ⬅️ NEW: callback (presentation, isSaved)
}) {
  const { data: session } = useSession();
  const [imgOk, setImgOk] = useState(true);
  const [saved, setSaved] = useState(
    initiallySaved || Boolean(p?.is_saved ?? p?.saved ?? false)
  );
  const [busy, setBusy] = useState(false); // ⬅️ NEW: prevent rapid double taps

  const router = useRouter();
  const ref = useRef(null);
  const href = `/presentations/${p?.slug ?? ""}`;

  // Prefetch the target route when near viewport
  useEffect(() => {
  ...
  // Image band height (~56% of the card height)
  ...
  const doToggleSave = useCallback(async () => {
    if (busy) return;
    if (!session) {
      // Not logged in → toast
      toast.info("Please log in to save this presentation.");
      return;
    }
    ...
    try {
      const res = await fetch("/api/my-lessons", {
        ...
        body: JSON.stringify({
          id: p?.id, // prefer numeric id if you have it
          slug: p?.slug, // fallback to slug
          topic: p?.topic,
          sub_topic: p?.sub_topic,
          subject: p?.subject,
          grade: p?.grade,
          thumbnail: p?.thumbnail || p?.thumbnailUrl || p?.thumbnail_path || "",
        }),
      });

      if (res.status === 401) {
        // not logged in — revert UI and notify
        setSaved(!next);
        toast.info("Please log in to save the presentation.");
        setBusy(false);
        return;
      }
      ...
      // success → notify parent (e.g., My Lessons page can remove it instantly)
      onSavedChange?.(p, next);

      if (next) toast.success("Saved to My Lessons");
      else toast.success("Removed from My Lessons");
    } catch (e) {
      // rollback
      setSaved(!next);
  ...
  }, [busy, session, saved, p, onSavedChange]);
  ...
  return (
    ...
    >
      ...
      <button
        type="button"
        aria-label={saved ? "Unsave presentation" : "Save presentation"}
        className={`absolute right-2 top-2 z-10 rounded-full ${busy ? "opacity-60 pointer-events-none" : ""}`}
        onClick={(e) => { e.preventDefault(); e.stopPropagation(); doToggleSave(); }}
      >
      ...
      <div className="p-4 flex flex-col flex-1 min-h-0">
        <h3 className="font-inter line-clamp-2 text-[#334155] font-bold">
          {p?.topic || p?.name || "Untitled presentation"}
        </h3>
