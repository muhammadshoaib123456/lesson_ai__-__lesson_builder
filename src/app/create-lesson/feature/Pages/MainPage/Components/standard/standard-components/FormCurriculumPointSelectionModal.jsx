...
export const TreeNode = ({
  ...
  const isSelected = selectedPoints.some((point) => point.point_id === item.point_id);
  const canSelect = selectedPoints.length < maxSelections || isSelected;

  // Format display text
  const displayText = item.code ? `${item.code}: ${item.description}` : item.description;
  ...
  return (
    <div className="w-full my-2">
      <div
        ...
        } ${depth === 1 ? "bg-gray-50" : ""} ${isSelected ? "bg-purple-100 border border-purple-300" : ""}`}
  ...
  return (
    <div className="space-y-3">
      {suggestedPoints.map((point) => {
        const isSelected = selectedPoints.some((p) => p.point_id === point.point_id);
        const canSelect = selectedPoints.length < maxSelections || isSelected;
        return (
          <div
            key={point.point_id}
          ...
          >
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-sm font-medium text-purple-600">{point.code}</span>
              </div>
                ...
                {selectedPoints.map((point, index) => (
                  <div key={point.point_id} className="flex items-start gap-2">
                    ...
                    <p className="text-sm text-purple-800">
                      {point.code ? `${point.code}: ${point.description}` : point.description}
                    </p>
                  </div>
                ))}
              </div>
