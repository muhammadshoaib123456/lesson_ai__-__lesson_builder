...
export const TreeNode = ({
  ...
  const handleToggle = () => {
    if (hasChildren) toggleNode(item.point_id);
  };

  const handleCheckboxChange = () => {
    if (isLeaf) onPointToggle(item);
  };
  ...
  return (
    <div className="w-full my-2">
      ...
      >
        ...
        <span
          className={`text-sm ${
            isLeaf ? "text-purple-700" : "text-gray-700"
...
export function FormCurriculumPointSelectionModal({
  ...
  const [currentView, setCurrentView] = useState("suggested");
  ...
  return (
    <div className="fixed inset-0 w-full h-full bg-black/30 backdrop-blur-sm flex overflow-y-auto justify-center items-center z-50">
      ...
      >
        {/* Header */}
        <div className="px-6 py-4 bg-purple-600 rounded-t-xl flex justify-between items-center">
        ...
        <div className="flex flex-col w-full p-6 gap-4 overflow-y-auto flex-1">
          <p className="text-gray-600 text-sm">
            {currentView === "suggested"
              ...
              : `Choose up to ${maxSelections} curriculum points. Click the arrows to expand categories and select specific points.`}
          </p>
